/* qmatch.c - FIFO Queue Match for erandu  Version 1.0.0               */
...
void enque(qfmt *tail, unsigned int outpt)
   ...
   newnode->prev = tail;
   /* The outpt parameter is the output of an erandu random number */
   /* generator cycle */
   newnode->outpt = outpt;
   } /* enque */
...
void deqenq(qfmt *head, qfmt *tail, unsigned int outpt)
   ...
   /* erandu random number generator cycle */
   node->outpt = outpt;
   } /* deqenq */
...
int main()
   ...
   unsigned int outpt;         /* 32-bit output from an erandu cycle */
   ...
   while (1)         /* infinite loop */
	 ...
	 char str[128];
         move(5,24);
         sprintf(str,"erandu with Bays-Durham Shuffle");
         addstr(str);
	 move(10,27);
